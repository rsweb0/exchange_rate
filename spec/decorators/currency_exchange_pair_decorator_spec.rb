...
RSpec.describe CurrencyExchangePairDecorator do
  ...
  it 'should return historic data when base_currency is EUR' do
    ...
    expect(object.decorate.historic_data).to eq([[@cur_exchange.date, @cur_exchange.rate],
                                                 [@exchange_one.date, @exchange_one.rate]])
  end
  ...
  it 'should return historic data when base_currency is not EUR' do
    ...
    expect(obj.decorate.historic_data).to eq([[Date.today, 60.0], [1.days.ago.to_date, 65.0]])
  end
